<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($post_remember_me)) {
            ...
            setcookie('lop_student_auth_user',   $post_auth_user,   time() + 3600, "/");
            setcookie('lop_student_auth_pass',   $post_auth_pass,   time() + 3600, "/");
            setcookie('lop_student_remember_me', $post_remember_me, time() + 3600, "/");
        } else {
            // Empty cookies which lasts for an hour.
            if (isset($_COOKIE['lop_student_auth_user']))   setcookie('lop_student_auth_user',   '', time() + 3600, "/");
            if (isset($_COOKIE['lop_student_auth_pass']))   setcookie('lop_student_auth_pass',   '', time() + 3600, "/");
            if (isset($_COOKIE['lop_student_remember_me'])) setcookie('lop_student_remember_me', '', time() + 3600, "/");
        }
        
        // $conn = mysqli_connect($db_server, $db_username, $db_password, $db_name);
        // if ($conn) {
        //     $post_id_user_type = 1;
        //     $stmt = $conn->prepare($query_select_login);
        //     $stmt->bind_param("si", $post_auth_user, $post_id_user_type);
        //     $stmt->execute();
        //     $stmt->bind_result($out_id, $out_auth_pass);
        //     $stmt->store_result();
        //     $stmt->fetch();
        //     $rows = $stmt->num_rows;
        //     $stmt->close();
            
        //     // Proceed if there are rows otherwise go to else.
        //     if ($rows > 0) {
        //         if (crypt($post_auth_pass, $out_auth_pass) == $out_auth_pass) {
        //             // Store session data.
        //             $_SESSION['is_login']     = 1;
        //             $_SESSION['id_user']      = $out_id;
        //             $_SESSION['id_user_type'] = $post_id_user_type;
        ...
        //             session_destroy();
        //             $err_msg = "Invalid password specified.";
        //         }
        //     } else {
        //         // Destroy session then initialize error message.
        //         session_destroy();
        ...
        //     mysqli_close($conn);
        // }
        include('class/loginstudent.php');

        if($login->LoginAuthStudent($post_auth_user,$post_auth_pass)){

            if($login->GetIdStudent($post_auth_user,$post_auth_pass)){
                foreach($login->GetIdStudent($post_auth_user,$post_auth_pass) as $row){
                    $id = $row['id'];
                }
...
<html lang="en">
    <head>
        <title>LPU OJT PORTAL - Student Login</title>
        <meta charset="utf-8">
    ...
    <body background="./assets/images/portal_bg.png">
        ...
        <div class="container-fluid">
            <div class="portal-content">
                ...
                <form action="./login_student.php" method="POST" style="margin-top:16px">
                    <div class="form-row row">
                        <label class="h6" for="auth_user">Student ID</label>
                        <input class="form-control" type="text" name="auth_user" id="auth_user" placeholder="Enter student id" value="<?php if (isset($_COOKIE["lop_student_login"])) { print($_COOKIE["lop_student_login"]); } ?>" required autofocus />
                    </div>
                    <div class="form-row row" style="margin-top:8px">
                        <label class="h6" for="auth_pass">Password</label>
                        <input class="form-control" type="password" name="auth_pass" id="auth_pass" placeholder="Enter password" value="<?php if (isset($_COOKIE["lop_student_password"])) { print($_COOKIE["lop_student_password"]); } ?>" required />
                    </div>
                    <div class="form-row row" style="margin-top:8px">
                        <div class="col-12">
                            <div class="form-check text-center">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="checkbox" name="remember_me" <?php if (isset($_COOKIE["lop_student_remember_me"])) { ?>checked<?php } ?> />&nbsp;Remember Me
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- <div class="form-row row" style="margin-top:8px">
                        <div class="col-12">
                            <a class="btn btn-primary" href="./apply_ojt_internship.php">Apply OJT Internship</a>
                        </div>
                    </div> -->
                </form>
            </div>
