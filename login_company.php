<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($post_remember_me)) {
            ...
            setcookie('lop_company_auth_user',   $post_auth_user,   time() + 3600, "/");
            setcookie('lop_company_auth_pass',   $post_auth_pass,   time() + 3600, "/");
            setcookie('lop_company_remember_me', $post_remember_me, time() + 3600, "/");
        } else {
            // Empty cookies which lasts for an hour.
            if (isset($_COOKIE['lop_company_auth_user']))   setcookie('lop_company_auth_user',   '', time() + 3600, "/");
            if (isset($_COOKIE['lop_company_auth_pass']))   setcookie('lop_company_auth_pass',   '', time() + 3600, "/");
            if (isset($_COOKIE['lop_company_remember_me'])) setcookie('lop_company_remember_me', '', time() + 3600, "/");
        }
...
<html lang="en">
    <head>
        <title>LPU OJT PORTAL - Company Login</title>
        <meta charset="utf-8">
            ...
            <div class="portal-content">
                <div class="row">
                    <div class="col text-center">
                        <img src="./assets/images/company_logo.png" width="96px" height="96px" />
                    </div>
                </div>
                <div class="row" style="margin-top:16px">
                    <div class="col text-center">
                        <p class="h3">Company Login</p>
                    </div>
                </div>
                <form action="./login_company.php" method="POST" style="margin-top:16px">
                    <div class="form-row row">
                        <label class="h6" for="auth_user">Username</label>
                        <input class="form-control" type="text" name="auth_user" id="auth_user" placeholder="Enter username" value="<?php if (isset($_COOKIE["lop_company_login"])) { print($_COOKIE["lop_company_login"]); } ?>" required autofocus />
                    </div>
                    <div class="form-row row" style="margin-top:8px">
                        <label class="h6" for="auth_pass">Password</label>
                        <input class="form-control" type="password" name="auth_pass" id="auth_pass" placeholder="Enter password" value="<?php if (isset($_COOKIE["lop_company_password"])) { print($_COOKIE["lop_company_password"]); } ?>" required />
                    </div>
                    <div class="form-row row" style="margin-top:8px">
                        <div class="col-12">
                            <div class="form-check text-center">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="checkbox" name="remember_me" <?php if (isset($_COOKIE["lop_company_remember_me"])) { ?>checked<?php } ?> />&nbsp;Remember Me
                                </label>
                            </div>
                        </div>
                    </div>
